...
def visualize_prediction(image, prediction, probability=None, mask=None, 
                        save_path=None, show_probability=True):
    """
...
def process_single_image(args, model, device, transform):
    ...
    if mask is not None:
        ...
        pred_tensor = torch.from_numpy(prediction_resized).unsqueeze(0).unsqueeze(0).float()
    ...
    visualize_prediction(
        image, prediction_resized, probability_resized, mask,
        save_path=viz_path, show_probability=args.show_probability
    )
    
    # Save overlay
    if args.save_overlay:
        overlay = overlay_prediction(image, prediction_resized, alpha=args.overlay_alpha)
...
def main():
    parser = argparse.ArgumentParser(description='U-Net Inference for Image Segmentation')
    ...
    parser.add_argument('--show_probability', action='store_true',
